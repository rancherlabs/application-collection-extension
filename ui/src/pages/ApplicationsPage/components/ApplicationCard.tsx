...
import { ApplicationReducedViewDTO, ApplicationReducedViewDTOTypeEnum } from '../../../../autogenerated/client/backend'
...
export default function ApplicationCard({ app }: { app: ApplicationReducedViewDTO }) {
  ...
  const comingSoon = !app.type

  function humanFriendlyType() {
    if (app.type === ApplicationReducedViewDTOTypeEnum.HelmChart) return 'Helm chart'
    if (app.type === ApplicationReducedViewDTOTypeEnum.SingleContainer) return 'Single container'
    return 'Coming soon'
  }
      ...
      <CardActionArea 
        ...
        <CardContent sx={ { p: 3 } }>
          <Stack direction={ { xs: 'column', sm: 'row' } } alignItems={ { xs: 'start', sm: 'center' } } flexWrap='wrap' rowGap={ 2 } >
            ...
            <Chip label={ humanFriendlyType() } size='small' color={ comingSoon ? 'default' : 'primary' } />
          </Stack>
          <Typography 
            variant='body2' 
            sx={ { mt: 2 } }>
