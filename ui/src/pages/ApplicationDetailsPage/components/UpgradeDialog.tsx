...
import { Notification, useNotificationsDispatch } from '../../../components/NotificationsCenter/NotificationsContext'
...
export default function UpgradeDialog({ artifact, workload, open, onDismiss = () => null }: 
{ artifact: ArtifactListItemReducedDTO, workload: HelmListItem, open: boolean, onDismiss?: () => any }) {
  ...
  useEffect(() => {
    if (open) {
      ...
      checkKubernetes(ddClient)
        .then(() => ddClient.extension.vm?.service?.get(`/charts/${artifact.name.split(':')[0]}/${artifact.version}/local-values`)
          .then(result => {
        ...
        .catch((e) => {
          console.error('Unexpected error checking kubernetes cluster', e)
          setState('error')
          setError('Cannot verify the target kubernetes cluster. Make sure it is up and reachable.')
        })
    }
  }, [open, artifact])
  ...
  function upgrade() {
    ...
    const notification: Notification = {
      id: uuid(),
      title: 'Updating ' + workload.name + ' to ' + artifact.version,
      description: 'Update ongoing in the background',
      type: 'progress',
      dismissed: false,
    ...
    upgradeHelmChart(ddClient, artifact, workload, values)
      .then(() => dispatch({
        ...
        payload: {
          ...notification,
          title: 'Updated ' + workload.name + ' to ' + artifact.version,
      ...
      .catch(() => dispatch({
        ...
        payload: {
          ...notification,
          title: 'Error updating ' + artifact.name,
          description: 'Open the workload details page to see more details about the error.',
          type: 'error',
          dismissed: false,
          timestamp: new Date().getTime(),
        }
